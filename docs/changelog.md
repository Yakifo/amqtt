...
- The `FileAuthPlugin` hash scheme has migrated from `sha512_crypt` to `argon2`. For this release, the `sha512_crypt` passwords will be accepted alongside `argon2` hashes. A `DeprecationWarning` is displayed, but automatic migration is not supported; see [FileAuthPlugin](plugins/packaged_plugins.md#FileAuthPlugin) for information on how to create a new password file.
