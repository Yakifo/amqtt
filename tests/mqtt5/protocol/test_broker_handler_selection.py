...
@pytest.mark.asyncio
async def test_init_handler_from_connect_selects_mqtt5_handler(make_reader, v5_connect_packet, mock_v5_session) -> None:
    broker = _make_broker()
    writer = BufferWriter()
    handler = object()
    session = mock_v5_session
    connect_packet = v5_connect_packet.to_bytes()
    ...
    with (
        ...
        selected_handler, selected_session = await broker._init_handler_from_connect(make_reader(connect_packet), writer)  # noqa: SLF001
...
async def test_init_handler_from_connect_keeps_non_mqtt5_on_mqtt3_handler(connect_packet: bytes, make_reader) -> None:
    broker = _make_broker()
    writer = BufferWriter()
    handler = object()
    session = Session()
