...
def bytes_to_int(data: bytes | int) -> bytes | int:
...
async def read_or_raise(reader: ReaderAdapter, n: int = -1) -> bytes:
...
async def decode_packet_id(reader: ReaderAdapter) -> int | Any:
